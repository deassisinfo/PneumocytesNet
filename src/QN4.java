...
public class QN4 extends QN {
	
	public static final int N_ITER = 30;
	public static final int QN_SIZE = 52;
	
	public QN4(Equation[] equations){
		this(equations, N_ITER);
	}

	public QN4(Equation[] equations, int iterations){
		...
		for(int i = 0; i < iterations; i++) {
			...
			for(int caseName : net) {
			    ...
			    case ADAM_17:
			        ...
			        break;
			    case AKT:
			    	x[AKT] = act(ko[2], AND(NOT(x[MK2206]), f(AVG(x[idx[mTORC2_3]] - v[4], x[idx[PI3K_4]] - v[5], x[idx[IKK_a_b_5]] - v[6]) - AVG(x[idx[PTEN_6]] - v[7], x[idx[Viral_Repl_109]]), x[AKT])));
			    ...
			    case IFNR:
			        ...
			        break;
			    case IL6:
			    	x[IL6] = act(ko[17], OR(x[IL6_e], il6.compute(x, idx)));
			        break;
			    case IL6R:
			        x[IL6R] = act(ko[18], f(AVG(x[idx[IL6_88]] - v[48]), x[IL6R]));
			        break;
			    case IRF3:
			        x[IRF3] = act(ko[19], f(AVG(x[idx[RIG1_42]] - v[49], x[idx[TLR4_107]]), x[IRF3]));
			        break;
			    case IKK_a_b:
			    ...
			    case ISG:
			        ...
			        break; 
			    case MAPK_p38:
			        x[MAPK_p38] = act(ko[22], f(AVG(x[idx[ANG_2_T1R_48]] - v[55], x[idx[TLR4_49]] - v[56], x[idx[ROS_50]] - v[57]), x[MAPK_p38]));
			        break;
			    case mTORC1:
			    	//x[mTORC1] = act(ko[23], AND(NOT(OR(x[RAPAMICINA], x[PP242_e])), f(AVG(NOT(x[idx[TSC2_52]]) - v[59]), x[mTORC1])));
			        x[mTORC1] = act(ko[23], AND(NOT(OR(x[RAPAMICINA], x[PP242_e])), f(AVG(x[idx[IKK_a_b_51]] - v[58], NOT(x[idx[TSC2_52]]) - v[59]), x[mTORC1])));
			        break;
			    case mTORC2:
			    	//x[mTORC2] = act(ko[24], AND(NOT(x[PP242_e]), f(AVG(x[idx[PI3K_53]] - v[60], x[idx[AMPK_54]] - v[61]), x[mTORC2])));
			        x[mTORC2] = act(ko[24], AND(NOT(x[PP242_e]), f(AVG(x[idx[PI3K_53]] - v[60], x[idx[AMPK_54]] - v[61], x[idx[IKK_a_b_55]] - v[62]), x[mTORC2])));
			        break;
			    case NFKB:
			        x[NFKB] = act(ko[25], f(AVG(x[idx[IKK_a_b_56]] - v[63], x[idx[ROS_57]] - v[64]) - AVG(x[idx[FOXO3A_58]] - v[65], x[idx[Viral_Repl_108]]), x[NFKB]));
			    ...
			    case TNFR:
			        ...
			        break;
			    case TSC2:
			        x[TSC2] = act(ko[36], f(AVG(x[idx[AMPK_78]] - v[90], x[idx[HIF_1a_79]] - v[91]) - AVG(x[idx[AKT_80]] - v[92]), x[TSC2]));
			        break;
			    case Viral_Repl:
